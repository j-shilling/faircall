...
static void open_dialog (gpointer callback_data, guint callback_action,
			 GtkWidget *menu_item);
static void about_dialog (gpointer callback_data, guint callback_action,
			  GtkWidget *menu_item);
...
open_dialog (gpointer callback_data, guint callback_action,
		 GtkWidget *menu_item)
{
  GtkWidget *dialog = gtk_dialog_new_with_buttons ( "Open Class",
  ...
  class_t **classes = get_class_list();
  int i = 0;
  ...
  while (classes[i] != NULL)
    {
      gtk_list_store_set (store, &iter,
			  CLASS_ID, i,
			  NAME_COLUMN, class_get_name(classes[i]),
			  SIZE_COLUMN, class_get_size(classes[i]),
			  LAST_CALLED_COLUMN, class_get_last_called(classes[i]),
			  -1);
      i ++;
    }
  ...
  GtkTreeViewColumn *column = gtk_tree_view_column_new_with_attributes ("Class",
									renderer,
									"text", NAME_COLUMN,
									NULL);
  ...
  gtk_box_pack_start (GTK_BOX (action_area), tree,
		      TRUE, TRUE, 0);
